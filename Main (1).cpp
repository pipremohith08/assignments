...
struct order{
    ...
    void confirmorder(){
        if(pay==PAID){
            ...
            cout<<"Order cannot be confirmed because payment failed"<<endl;
        }
    }
...
struct Payment{
    ...
    Payment(PaymentMethod m, paymentStatus s, const char id[]){
        method=m;
        status=s;

        if(method==Upi){
            strcpy(info.upiId,id);
        }
    }
...
int main(){
    ...
    if(product2.checkavailibilty())
        cout<<"Laptop is available"<<endl;
    else
        cout<<"Laptop is out of stock"<<endl;
